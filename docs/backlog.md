...
- **[x] `09-relatorio-pdf.md`** — leva pro mobile a geração do relatório em PDF já implementada no web (`docs/specs/15-relatorio-pdf.md`). Spec aprovada e implementada (2026-08-04) — `expo-file-system`/`expo-sharing` adicionadas, type-check/testes/bundling passando. **Falta validar rodando no simulador/dispositivo de verdade** (mesma limitação registrada nas outras specs mobile — sem simulador disponível neste ambiente).
