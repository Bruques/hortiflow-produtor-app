...
## Adendo 2026-09-09 (3) — troca pro flash-lite e restrição por plano

- **Modelo trocado pra `gemini-3.5-flash-lite`** (decisão do dev, priorizando custo): teste comparativo real (mesma foto, mesmo prompt) mostrou ~R$0,05 por chamada contra ~R$0,12 do `gemini-3.6-flash`, mas deixou passar 1 de 8 lançamentos (o modelo anterior pegou os 8). Decisão consciente de aceitar essa diferença de qualidade — a revisão humana obrigatória na tela é a rede de segurança pro que passar batido.
- **Importação por IA vira recurso exclusivo de Plano 2 e Plano 3** (decisão do dev): quem está no Plano 1 ou ainda em trial (sem plano atribuído) recebe 403 ao tentar usar (`assinatura.service.ts` → `planoPermiteImportacaoPorIAParaSafra`, checado em `importacao.controller.ts` — esse é o bloqueio que vale de verdade)
- **Card do Menu (web e mobile) também reflete isso visualmente**: chama `GET /assinatura/status` (endpoint já existente da spec 18) ao carregar o Menu e, se o plano não for "Plano 2"/"Plano 3", mostra o card "Importar lançamentos" apagado com o subtítulo "Recurso do Plano 2+", sem navegar ao tocar. **Limitação conhecida e aceita pelo dev**: esse endpoint devolve o plano de quem está logado, não o do titular da sociedade — funciona certo pro financiador (~98% de quem usa o Menu, ver CLAUDE.md), mas um meeiro pode ver o card bloqueado mesmo a sociedade dele tendo Plano 2/3 (o bloqueio de verdade, no backend, sempre olha o titular certo — essa imprecisão é só visual). Revisitar se algum meeiro reclamar na prática.

## Adendo 2026-09-09 (2) — limite de tokens de "pensamento" (custo)

Depois de 4 testes reais em staging, o dev notou custo maior do que esperado (~R$0,25/chamada). Investigação (chamadas reais de teste, `usageMetadata` da resposta): o Gemini 3.x faz "thinking" (raciocínio interno) antes de responder, cobrado como token de saída (US$3,75/milhão, bem mais caro que entrada) — numa chamada trivial já eram ~230 tokens de pensamento; numa foto real com várias linhas ambíguas, plausivelmente alguns milhares, o que sozinho explica o custo observado.

Ajuste: `thinkingConfig: { thinkingBudget: 1024 }` em `importacao.service.ts`, limitando (não eliminando) o quanto o modelo pensa por chamada. Testado que `thinkingBudget: 0` (desligar completamente) retorna 400 nesse modelo — não é uma opção. 1024 foi escolhido como meio-termo: teto contra casos que disparariam o raciocínio, mas espaço suficiente pra várias linhas ambíguas numa página. **Ainda não validado com foto real** se esse limite prejudica a qualidade da extração em páginas complexas — validar em uso real e ajustar o número se a qualidade cair.
