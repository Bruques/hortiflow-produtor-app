...
- **URL real do backend `develop` obtida e testada** (`https://backend-develop-274f.up.railway.app`) — descoberto nesse processo que as rotas do backend são montadas sob o prefixo `/api` (`app.ts`), não na raiz como os contratos das specs sugerem à primeira vista. Seguido o mesmo padrão já usado no frontend web (`VITE_API_URL` já inclui `/api`): `EXPO_PUBLIC_API_URL` também inclui o `/api` no valor da variável, então os `services/*.ts` continuam chamando rotas relativas (`/auth/login`) sem precisar saber do prefixo.
- **Fila de sincronização passou a enviar `idempotency_key` nas criações que geram entidade nova (venda, despesa)** (bug relatado 2026-08-11, corrigido nesta data): o `localId` que a fila já gerava pra reconciliar o cache local nunca era enviado ao backend. Quando o `POST` era processado com sucesso no servidor mas a resposta HTTP se perdia (timeout/instabilidade — cenário comum do produtor sem sinal bom), o cliente entendia que a operação falhou, deixava o item preso em status `erro` (retentado a cada sincronização) e a próxima listagem trazia o registro real do servidor como uma segunda entrada — resultado: total exibido em duplicidade, e uma entrada "fantasma" que nunca saía da tela. Correção: o `localId` agora é enviado como `idempotency_key` no payload de criação; `Venda` e `Despesa` ganharam uma coluna `idempotency_key` (única) no schema, e o backend, ao receber um retry com uma chave já usada, devolve o registro já existente em vez de criar um novo. Vale só pra criação — edição (PUT) e exclusão (DELETE) já eram idempotentes por natureza (operam sobre um id de servidor já conhecido, e um 404 é tratado como sucesso silencioso).
- **Design visual portado do web** (fora do escopo original desta spec, mas registrado aqui por ser infraestrutura compartilhada, mesmo padrão da fila de sincronização): `mobile/src/theme.ts` (paleta/espaçamento/tipografia) e `mobile/src/components/BrandMark.tsx` (ícone SVG + wordmark, via `react-native-svg`), com `lucide-react-native` para ícones de interface. Documentado em `docs/design/notas-de-design.md` (seção "Mobile") e referenciado no `CLAUDE.md`. A tela de Login foi restilizada como primeira aplicação; a fonte `ui-rounded` do web não tem equivalente nativo direto sem `expo-font` — fica registrado como pendência em aberto no doc de design, não resolvida agora.
