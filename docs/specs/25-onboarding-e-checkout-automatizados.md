...
- **Cartão anual**: o app **redireciona pro checkout hospedado do Mercado Pago** (abre em navegador/WebView, via Checkout Pro / API de Preferências) — o produtor sai do app nesse momento, paga na página do Mercado Pago, e volta depois. **Cobrança única, à vista** — não faz sentido débito automático num ciclo que já é pago uma vez só por ano; o produtor paga de novo manualmente daqui a 1 ano. O parcelamento em até 12x, se o banco do produtor oferecer, acontece do lado da operadora do cartão dele, não é uma assinatura recorrente nossa
- **Cartão mensal — débito automático de verdade (retomado em 2026-09-16, a pedido do dev)**: diferente do anual, o cartão mensal **é** uma assinatura recorrente — o produtor autoriza uma vez e é cobrado automaticamente todo mês até cancelar. Sem redirecionamento: o cartão é **tokenizado dentro do próprio checkout** (número, validade, CVV e nome digitados num formulário nosso; CPF do titular também obrigatório aqui, diferente do Pix onde é opcional) usando a **Secure Fields** do Mercado Pago (`MercadoPago.js` v2) — o dado sensível do cartão nunca passa pelo nosso servidor, só o token final. Web: formulário embutido direto na tela de checkout (`CartaoTokenForm.tsx`). Mobile: como não existe SDK React Native oficial do Mercado Pago, o mesmo formulário roda numa **página própria do frontend web** (`/assinatura/tokenizar-cartao`, pública, sem login) aberta dentro de uma **WebView** do app, que devolve o token pro app nativo via `postMessage`. Ver "Perguntas em aberto" sobre por que a primeira tentativa (redirecionamento pra página hospedada de assinatura) foi abandonada e como essa segunda tentativa evita o mesmo problema
- **Cancelamento do cartão mensal recorrente**: exigência legal (Decreto 11.034/2022, art. 14, que regulamenta o CDC pro comércio eletrônico) — cancelamento tem que poder ser feito pelo mesmo canal da contratação, com efeito imediato. Já existe: tela "Minha assinatura" (spec 18, sem mudança) com botão de cancelar de um clique, que chama `PUT /preapproval/:id` do Mercado Pago (para a recorrência na hora) e marca `CANCELADA` no nosso banco — o acesso já pago continua até `dataFimAcesso`, sem cobrança futura
...
POST /assinatura/checkout
  Body: { planoId, ciclo: "MENSAL"|"ANUAL", metodo: "CARTAO"|"PIX", retornoUrl?: string, cardTokenId?: string }
    // `retornoUrl` é pra onde o Mercado Pago volta depois do pagamento (só usado por cartão
    // anual, que redireciona) — cada cliente manda a sua (deep link do app mobile, URL do
    ...
    // `cardTokenId` é obrigatório quando ciclo=MENSAL e metodo=CARTAO (token gerado no
    // cliente via CartaoTokenForm/Secure Fields) — ignorado nos outros casos.
  ...
  200 (cartão mensal, assinatura recorrente): { tipo: "ASSINATURA", mpPreapprovalId, status }
    // sem redirecionamento nenhum — o acesso já libera nessa resposta se status="authorized"
  ...
  400: { error: "Plano ou ciclo inválido" } | { error: "Dados do cartão inválidos ou ausentes" }
...
8. Dado um usuário que escolhe plano mensal + cartão no checkout, quando tokeniza o cartão e confirma, então a assinatura recorrente é criada (`tipo: "ASSINATURA"`), o acesso libera na hora, e o cartão é cobrado automaticamente todo mês até o produtor cancelar
8b. Dado um usuário que escolhe plano anual + cartão no checkout, quando confirma, então recebe o link do checkout hospedado do Mercado Pago (`initPoint`) de uma cobrança única no valor anual — sem débito automático
8c. Dado um usuário no cartão mensal recorrente, quando cancela pela tela "Minha assinatura", então a recorrência para imediatamente no Mercado Pago, a assinatura vira `CANCELADA` no nosso banco, e o acesso já pago continua até `dataFimAcesso` (sem cobrança futura)
...
- **Cartão mensal como assinatura recorrente de verdade — abandonado em 2026-09-15, RETOMADO em 2026-09-16 com uma abordagem diferente**: a primeira tentativa usava `/preapproval` sem cartão tokenizado, que devolve um `init_point` redirecionando pra uma página hospedada do Mercado Pago (`/subscriptions/checkout?preapproval_id=...`) — essa página sempre retornava "Esta página não existe" nessa conta, mesmo depois de declarar "Assinaturas" como produto integrado, causa nunca identificada. A pedido do dev (2026-09-16), retomamos com a abordagem que a própria documentação do Mercado Pago recomenda pra evitar esse redirecionamento: tokenizar o cartão no próprio checkout (Secure Fields) e criar a assinatura direto com `card_token_id` + `status: authorized` — sem nunca navegar pra página que dava erro. Funciona tanto pra "assinatura sem plano associado" (o caminho escolhido, mais simples, já que os planos/preços já são nossos) quanto pra "assinatura com plano associado" (não usado). Fontes: [Visão geral de Assinaturas](https://www.mercadopago.com.br/developers/pt/docs/subscriptions/overview), [Assinaturas sem plano associado — pagamento autorizado](https://www.mercadopago.com.ar/developers/en/docs/subscriptions/integration-configuration/subscription-no-associated-plan/authorized-payments)
- **Acesso liberado antes da primeira cobrança de verdade (decisão de design, 2026-09-16)**: a documentação do Mercado Pago avisa que a primeira cobrança de uma assinatura recorrente só acontece **~1h depois** de autorizada — se a gente só liberasse acesso quando o webhook da primeira cobrança confirmasse (mesmo padrão usado em Pix e cartão anual), o produtor ficaria barrado no gate 402 por até 1h logo depois de assinar, o que é uma experiência ruim. Decisão: `iniciarCheckout` já estende `dataFimAcesso` (+30 dias) e marca `ATIVA` assim que `status: authorized` volta da criação da assinatura — quando o webhook da primeira cobrança chegar de fato, soma mais 30 dias em cima (mesma lógica idempotente de qualquer renovação, sem duplicar `Pagamento`). Risco aceito: se a autorização "vingar" mas a cobrança de fato falhar (raro — `authorized` já significa que o Mercado Pago validou o cartão o suficiente pra tentar cobrar), o produtor ganha até 30 dias sem ter pago — nenhum mecanismo de estorno automático desse cenário específico foi implementado
- **Webhook da assinatura recorrente reaproveita o de pagamento normal, sem mudança**: diferente do que se poderia imaginar, cada cobrança mensal de uma assinatura recorrente não tem um tipo de webhook próprio pra confirmar — ela gera um pagamento normal (`type=payment`), então `confirmarPagamentoWebhookMercadoPago` (já existente, sem alteração) processa a segunda cobrança em diante do mesmo jeito que processa qualquer cobrança avulsa
