...
- No checkout: plano e ciclo vêm pré-selecionados do que o produtor escolheu na tela de plano, mas podem ser trocados ali; depois escolhe a forma de pagamento (Pix ou cartão). Ao confirmar, o app **redireciona pro checkout hospedado do Mercado Pago** (abre em navegador/WebView) — o produtor sai do formulário nativo do app nesse momento, paga na página do Mercado Pago, e volta pro app depois
- **Mensal + cartão**: cria assinatura recorrente no Mercado Pago (cobrança automática todo mês)
- **Mensal + Pix**: gera cobrança Pix a cada ciclo; como Pix não tem débito automático, o produtor recebe um novo QR Code/código copia-e-cola a cada mês e precisa pagar manualmente — se não pagar até o vencimento, cai no mesmo gate 402 de acesso vencido (sem lembrete automático nem tela diferenciada nesta spec)
- **Anual + cartão**: cobrança única do valor total anual (R$ 538,92 / R$ 970,92 / R$ 1.402,92), à vista no Mercado Pago — o parcelamento em até 12x, se o banco do produtor oferecer, acontece do lado da operadora do cartão dele, não é uma assinatura recorrente nossa
- **Anual + Pix**: cobrança única do valor total anual, à vista, sem parcelamento (Pix não parcela)
